...
var lsSet = function(key, value) {
    try {
        ...
        Toast.toast("Could not save data: storage full. See Settings > Delete to clear space.");
        CloudAll.abort = true;
        return false;
    }
};
...
var LocalStore = function(successCallback, errorCallback) {
    ...
    this.addCloud = function(myCloud, callback) {
        ...
        var ret = lsSetObj("cloud" + myCloud.cloudId, myCloud, "CloudQueue");
        callback(ret);
    };
    ...
    this.addCloudBlob = function(myCloudBlob, callback) {
        ...
        var ret = lsSetObj("cloudBlob" + myCloudBlob.localId, myCloudBlob, "CloudBlob");
        callback(ret);
    };

    this.addAward = function(myAward, callback) {
        var ret = lsSetObj("award" + myAward.id, myAward, "Awards");
        callback(ret);
    };
    ...
    this.addAwardEarned = function(awardEarned, callback) {
        ...
        var ret = lsSetObj("awardEarned" + awardEarned.earnedId, awardEarned, "AwardsEarned");
        callback(ret);
    };
    ...
    this.addGame = function(game, callback) {
        ...
        //console.log(game);
        var ret = lsSetObj("game" + game.id, game, "Games");
        callback(ret);
    };

    this.addOldGame = function(game, callback) {
        var ret = lsSetObj("oldgame" + game.id, game, "OldGames");
        callback(ret);
    };
    ...
    this.addPlayer = function(player, callback) { //TODO add cloud ID
        ...
        var ret = lsSetObj("player" + player.id, player, "Players");
        callback(ret);
    };

    this.addFaction = function(faction, callback) {
        var ret = lsSetObj("faction" + faction.id, faction, "Factions");
        callback(ret);
    };

    this.addTeam = function(team, callback) {
        var ret = lsSetObj("team" + team.name, team, "Teams");
        callback(ret);
    };

    this.addLocation = function(location, callback) {
        var ret = lsSetObj("location" + location.name, location, "Locations");
        callback(ret);
    };

    this.addPaused = function(details, callback) {
        var ret = lsSetObj("paused" + details.id, details, "Paused");
        callback(ret);
    };
    ...
    this.addScore = function(score, callback) {
        ...
        //console.log(score);
        var ret = lsSetObj("score" + score.scoreId, score, "Scores");
        callback(ret);
    };

    this.addSession = function(session, callback) {
        //console.log("Saving session: " + session.sessionId);
        var ret = lsSetObj("session" + session.sessionId, session, "Sessions");
    ...
    this.deletePlayerById = function(player_id, callback) {
        ...
        if (p !== null) {
            p.hidden = true;
            ret = lsSetObj("player" + player_id, p, "Players");
        }
        callback(ret);
    };
    ...
    this.deleteGameById = function(game_id, callback) {
        ...
        if (g !== null) {
            g.hidden = true;
            ret = lsSetObj("game" + game_id, g, "Games");
        }
        callback(ret);
    };
    ...
    this.updatePlayerPhoto = function(player_id, player_icon, callback) {
        ...
        p.icon = player_icon;
        p.iconURL = "BLOB";
        var ret = lsSetObj("player" + player_id, p, "Players");
    ...
    this.updateSessionPhoto = function(session_id, game_photo, callback) {
        ...
        if (s !== undefined && s !== null) {
            ...
            //console.log(s);
            ret = lsSetObj("session" + session_id, s, "Sessions");
        }
        if (callback) {
            callback(ret);
        }
